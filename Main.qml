...
Item {
  ...
  function geminiCommand(kind) {
    var localBin = root.home + "/.local/bin/omarchy-agent-usage-gemini"
    var bundledBin = Qt.resolvedUrl("bin/omarchy-agent-usage-gemini").toString().replace(/^file:\/\//, "")
    var args = []
    if (kind === "force") args.push("--force")
    if (kind === "limits") args.push("--limits-only")
    var script = 'if [ -x "' + localBin + '" ]; then exec "' + localBin + '" "$@"; elif [ -x "' + bundledBin + '" ]; then exec "' + bundledBin + '" "$@"; else exec omarchy-agent-usage-gemini "$@"; fi'
    var cmd = ["sh", "-c", script, "--"]
  ...
  function runUpdate(kind, agentIds) {
    if (!geminiProcess.running && providerEnabled("gemini")) {
      geminiProcess.command = geminiCommand(kind)
  ...
  function providerHasData(p) {
    if (alwaysShow && (p.providerId === "claude" || p.providerId === "gemini")) return true
